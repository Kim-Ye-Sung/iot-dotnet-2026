...
1. 새 프로젝트 - [위치](./winapp/IoT02WinSolutions/)
...
11. 디자이너화면에서 `F7` <--> 비하인트코드 `Shift+F7`
12. `Ctrl+Space`, `Alt+Enter` VS(VS Code 포함) 가장 많이 쓰이는 단축키
...
- Visual Studio 2022 이상 윈폼즈 개발시 디자인 화면에서 버튼을 더블클릭 이벤트 추가할 경우 발생하는 오류
- Designer.cs에 생성된 이벤트 선언문에 대한 .cs파일 이벤트핸들러가 생성되지 않아서 발생

#### 첫번째 방법
1. Designer.cs 내 `Window Form Designer generated code` 영역을 확장
...
- 속성 변경방법
    ...
    - 런타임 변경 : 비하인드 코드내에서 속성값을 변경. 실행시 변경되는 것
...
- 예전에는 따로 설치, 내 프로젝트에 복사 붙여넣기
- NuGet Package 존재 - Python pip와 동일한 기능

#### NuGet 설치 순서
1. 프로젝트 마우스 오른쪽 버튼 > NuGet 패키지관리 크릭
...
2. DatabaseHelper 클래스 생성, 작성
    - Select 메서드 작성 - [소스](./winapp/IoT02WinSolutions/DotNet06DbBooksApp/DatabaseHelper.cs)
